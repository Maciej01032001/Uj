...
class Triangle:
    ...
    def __str__(self):
        string_1=(self.pt1.x,self.pt1.y,self.pt2.x,self.pt2.y,self.pt3.x,self.pt3.y)
        return string_1

    def __repr__(self):
        string_2=Triangle.__name__,(self.pt1.x,self.pt1.y,self.pt2.x,self.pt2.y,self.pt3.x,self.pt3.y)
        return string_2# "Triangle(x1, y1, x2, y2, x3, y3)"

    def __eq__(self, other):
        if ((self.pt1.x==other.pt1.x and self.pt1.y==other.pt1.y )):
            return bool(1)
        else:
            return bool(0)
...
class TestTriangle(unittest.TestCase):
    ...
    def test_equal(self):
        self.assertEqual(self.trojkat5.equal(Triangle(3,1,1,1,2,3)),True)
        self.assertEqual(self.trojkat5.equal(Triangle(3, 1, 2, 3, 1, 1)), True)
        self.assertEqual(self.trojkat5.equal(Triangle(3, 1, 2, 3, 1, 2)), False)
